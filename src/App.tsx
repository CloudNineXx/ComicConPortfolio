...
export default function App() {
  ...
  useEffect(() => {
    ...
    const setFavicon = () => {
      ...
      document.title = 'Swee Ming | Tech Superhero Portfolio';
    };
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      >
        <nav className={`w-full bg-black backdrop-blur-md border-b-4 border-yellow-400 transition-all duration-500 transform ${
          navbarVisible ? 'translate-y-0 bg-opacity-75' : '-translate-y-full bg-opacity-50'
        }`}>
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3 md:py-4">
          <div className="flex justify-between items-center">
            {/* Simple Comic Logo */}
            <div className="relative flex items-center gap-2">
              <span className="hidden sm:inline text-yellow-400 text-xl font-black transform -rotate-2 transition-transform hover:rotate-0" style={{
                textShadow: '2px 2px 0px #000, -1px -1px 0px #000, 1px -1px 0px #000, -1px 1px 0px #000',
                WebkitTextStroke: '1px #000'
              }}>
                COMIC-CON INSPIRED
              </span>
              <span className="sm:hidden text-yellow-400 text-lg font-black transform -rotate-2" style={{
                textShadow: '2px 2px 0px #000, -1px -1px 0px #000, 1px -1px 0px #000, -1px 1px 0px #000',
                WebkitTextStroke: '1px #000'
              }}>
                SM COMIC
        ...
        {/* Mobile Menu Overlay */}
        <div className={`md:hidden fixed inset-0 bg-black bg-opacity-95 backdrop-blur-lg transition-all duration-300 ${
              ...
              {[
                ...
                >
                  <span className="flex items-center gap-3 sm:gap-4">
                    ...
                    <span className="text-lg sm:typography-heading-sm">{item.name}</span>
                  </span>
                </Button>
              ))}
            </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <ScrollReveal direction="up" delay={100}>
            <div className="mb-3 sm:mb-4">
              ...
              <p className="text-base sm:text-lg lg:typography-body-xl text-gray-300 max-w-3xl mx-auto">Learning and growing through collaborative innovation</p>
            </div>
          </ScrollReveal>
      ...
      <div className="hidden sm:block fixed top-1/4 left-2 sm:left-4 text-xl sm:text-2xl z-10 pointer-events-none opacity-40 sm:opacity-60">💥</div>
      <div className="hidden sm:block fixed bottom-1/4 right-2 sm:right-4 text-xl sm:text-2xl z-10 pointer-events-none opacity-40 sm:opacity-60">⚡</div>
    </div>
  );
}